/**
 * Based on https://github.com/zeplin/SURGICAL-native-extension/blob/develop/src/constants.js
 */

export const MAX_BRIGHTNESS = 255;
export const HUE_MAX_DEGREE = 360;
export const MAX_PERCENTAGE = 100;
export const HEX_BASE = 16;
export const JSON_SPACING = 4;
export const NUMERICAL_FONT_BOLD = 700;
export const NUMERICAL_FONT_NORMAL = 400;
export const SURGICAL_RULES_WITH_COLOR = [
    "backgroundColor", "borderColor", "color", "shadowColor", "textDecorationColor", "textShadowColor"
];
export const HTML_TAGS = [
    "a", "abbr", "address", "area", "article", "aside", "audio", "b", "base", "bdi",
    "bdo", "blockquote", "body", "br", "button", "canvas", "caption", "cite", "code",
    "col", "colgroup", "data", "datalist", "dd", "del", "details", "dfn", "dialog",
    "div", "dl", "dt", "em", "embed", "fieldset", "figcaption", "figure", "footer",
    "form", "h1", "h2", "h3", "h4", "h5", "h6", "head", "header", "hgroup", "hr",
    "html", "i", "iframe", "img", "input", "ins", "kbd", "keygen", "label", "legend",
    "li", "link", "main", "map", "mark", "math", "menu", "menuitem", "meta", "meter",
    "nav", "noscript", "object", "ol", "optgroup", "option", "output", "p", "param",
    "picture", "pre", "progress", "q", "rp", "rt", "ruby", "s", "samp", "script",
    "section", "select", "slot", "small", "source", "source", "span", "strong", "style",
    "sub", "summary", "sup", "svg", "table", "tbody", "td", "template", "textarea",
    "tfoot", "th", "thead", "time", "title", "tr", "track", "u", "ul", "const", "video", "wbr"
];

export const LAYER_TYPE_TAGNAME = {
    'text': 'p',
    'shape': '__DEFAULT__',
};

export const OPTION_NAMES = {
    COLOR_FORMAT: "colorFormat",
    SHOW_DIMENSIONS: "showDimensions",
    SHOW_DEFAULT_VALUES: "showDefaultValues",
    DEFAULT_TAG_NAME: "defaultTagName",
};